...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    pass
    TeTr = -2 * np.dot(X,self.X_train.T)#500 * 5000
    TeTe = np.dot(X, X.T)#500 * 500
    TrTr = np.dot(self.X_train, self.X_train.T)#5000 * 5000
    TrTrdiag = np.diag(TrTr)#1 * 5000
    TeTediag = np.diag(TeTe)#1 * 500
    TeTr = TeTr + TrTrdiag
    dists = np.sqrt((TeTr.T + TeTediag).T)
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      row = dists[i,:]
      row = np.argsort(row)
      closest_y = self.y_train[row[0:k]]
      pass
      #########################################################################
      # TODO:                                                                 #
